...
from transformers import AutoModelForSequenceClassification, AutoTokenizer, BertTokenizer , AutoModelForQuestionAnswering
...
def word_compare(text,freetext,Label,res2):  
    res=[]    
    text=text.lower().split(" ")
    for i in text:         
          for l in range(len(freetext)):
              x=freetext.split(" ")
              for j in x: 
                  if i==j:             
                    res.append(freetext) 
...
def email_Error():
    ...
    receiver_address =  'ankit.2.kumar@atos.net,atossyntel28@gmail.com'
   ...
   # "InsuredSum_sop",  "Insured Sum",
    def process():
        ...
        Label = ["policyExpiryDate", "premiumDueDate",
                    ...
                    "primaryPhoneNumber", "PaymentDue_sop", "PremiumValue_sop", "TaxAmount_sop",
        ...
        freetext = ["Policy Expiry Date", "Premium Due Date", "Total Premium Paid",
                    ...
                    "Primary Phone Number", "Payment Due", "Premium Value", "Tax Amount",
                    "Installment Frequency","insuredsum", "Policy Number"]

        newtext = ["expiry", "due Date", "total paid",
                    "inception", "registered email", "customer name",
                    "primary phone number", "due", "value", "tax amount",
                    "frequency","insuredsum", "policy"]            
        ...
        res2 = {newtext[i]: Label[i] for i in range(len(freetext))} 
        compare_list=[]
        word_respo=[]
        for l in range(len(Label)):
                    compare_list.append(
                        word_compare(email_body, newtext[l],Label[l],res2))
        print(compare_list)
        for val in compare_list:
            if val != None:
                word_respo.append(val)      
        print(word_respo)
        score = dict()
        finalList=[] 
        ...
        for i in word_respo:
            finalList.append(i)
        print(finalList)  
        score={finalList[i]:"value" for i in range(len(finalList))}     
        print(score) 
        ...
        def getPOService(resp, score):
            ...
            #selecting sop entity
            search="sop"
            lword=len(search)
            
            for i in polist:
                start_index=i.find(search)
                if(search== i[start_index:start_index+lword]):
                    sop.append(i)
            print(sop)        
            #
            
            for key, value in score.items():
                output.append(postResponse(url, p_no, key))
                polist.append(key)
            # for word in word_respo:
            #     output.append(postResponse(url, p_no, word))
            #     polist.append(word)   
            
            #polists=list(set(polist))    
            #print(polists)
            res = {polist[i]: output[i] for i in range(len(polist))}
            res= dict(res)
